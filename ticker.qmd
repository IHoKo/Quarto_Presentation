...
format:
  revealjs:
    ...
    footer: "Ticker · one machine · 291 tests · under 10 hours of hands-on work"
...
**Specified, built, tested and deployed in under 10 hours of hands-on work.**
:::
...
How much working software can one person build in a couple of evenings?
...
So I ran the experiment on myself. This is Ticker. It's a live stock dashboard with an AI analyst built into it. I specified it, built it, tested it and deployed it — and the whole thing took under ten hours.
...
## I am not a software developer {.center}

[What the project needed · what I actually had]{.kicker}

::: {.split}
::: {}
::: {.chipgrid}
[Backend APIs]{.chip}[Real-time WebSockets]{.chip}[Server-sent events]{.chip}
[React + hooks]{.chip}[Client state]{.chip}[TypeScript]{.chip}
[SQL schema design]{.chip}[Database migrations]{.chip}[FIFO lot accounting]{.chip}
[Caching + rate limits]{.chip}[Docker]{.chip}[CI/CD]{.chip}
[Cloud deployment]{.chip}[Session auth]{.chip}[Secrets handling]{.chip}
[Design systems]{.chip}[Data visualisation]{.chip}[Test engineering]{.chip}
:::

[Eighteen things this project needed. **I had shipped maybe a third of them.**]{.caption}
:::

::: {}
::: {.card}
[What I brought]{.tag}[Judgement, not syntax]{.h4}

Knowing what the system had to do. Knowing what *correct* meant for a ledger. Being able to specify it precisely enough that someone else could build it — and being able to tell whether what came back was right.
:::

::: {.card style="margin-top:0.7em"}
[What I did not bring]{.tag}[The code itself]{.h4}

I could not have hand-written the WebSocket fan-out, the React state layer or the deployment pipeline. **I did not need to.** I needed to know they had to exist, and what good looked like.
:::
:::
:::
...
Now, one thing I should be upfront about, because it changes how you should read everything that follows.

<span style="color:#c1121f;font-weight:700">[SLOW DOWN. SAY THIS PLAINLY, NOT APOLOGETICALLY.]</span>

I'm not a software developer. I don't write production software for a living.

<span style="color:#c1121f;font-weight:700">[PAUSE — 2 SECONDS. LET THAT SIT.]</span>

<span style="color:#c1121f;font-weight:700">[GESTURE ACROSS THE GRID — DO NOT READ THE ITEMS OUT]</span>

And this is what the project needed. Eighteen different things. Real-time connections. React and client-side state. Database schema design. FIFO lot accounting. Docker, deployment, session authentication, secrets handling, a design system, test engineering.

I'd say I'd genuinely shipped maybe a third of that list before.

<span style="color:#c1121f;font-weight:700">[POINT AT THE TOP CARD ON THE RIGHT]</span>

What I did bring was judgement. I knew what the system had to do. I knew what *correct* means for a portfolio ledger, which is a surprisingly subtle question. And I could describe it precisely enough that something else could build it.

<span style="color:#c1121f;font-weight:700">[POINT AT THE LOWER CARD]</span>

What I couldn't do was write most of it. I could not have hand-written that WebSocket fan-out, or the React state layer, or the deployment pipeline.

<span style="color:#c1121f;font-weight:700">[EMPHASIZE THIS LINE]</span>

And it turns out I didn't need to. I needed to know they had to exist, and to recognise whether what came back was any good.

That distinction is really the whole talk. So — let me show you what came back.
:::
...
<span style="color:#c1121f;font-weight:700">[CALL BACK TO SLIDE 2]</span>

And go back to what I said at the start — I'm none of those things. Not one of those bars is a job I've held.

So the honest baseline is: two working weeks, spread across a small team, none of whom are me.
...
def series(unit):
    """Values, +error, -error and labels, in hours or developer-days."""
    if unit == "hours":
        vals  = [PLAN_MID_H, ACTUAL_H]
        plus  = [PLAN_HI_H - PLAN_MID_H, 0]
        minus = [PLAN_MID_H - PLAN_LO_H, 0]
        txt   = [f"{PLAN_LO_H:.0f}–{PLAN_HI_H:.0f} hours  ", f"{ACTUAL_H:.0f} hours  "]
    else:
        vals  = [PLAN_MID, ACTUAL_H / HOURS_PER_DAY]
        plus  = [PLAN_HI - PLAN_MID, 0]
        minus = [PLAN_MID - PLAN_LO, 0]
        txt   = [f"{PLAN_LO:.1f}–{PLAN_HI:.1f} days  ", f"{ACTUAL_H/HOURS_PER_DAY:.1f} days  "]
    return vals, plus, minus, txt

y = ["What the spec estimated<br><sub>seven phases, priced before any code</sub>",
     "What it took<br><sub>hands-on, measured</sub>"]
v, ep, em, txt = series("hours")
...
fig = go.Figure(go.Bar(
    ...
    textfont=dict(family="IBM Plex Mono, monospace", size=16, color=PAPER),
    hovertemplate="%{y}<br>%{x:.0f}<extra></extra>"))

fig.add_annotation(
    x=0.995, y=0.06, xref="paper", yref="paper", xanchor="right", yanchor="bottom",
    ...
    text=(f"<span style='font-size:34px;color:{BRASS}'><b>≈{SPEEDUP_MID:.0f}× faster</b></span>"
          f"<br><b>{REDUCTION*100:.0f}% less time</b>"
          f"<br><span style='font-size:13px;color:{MUTED}'>"
          f"{SPEEDUP_LO:.0f}× – {SPEEDUP_HI:.0f}× across the estimate's range<br>"
          f"one developer-day counted as {HOURS_PER_DAY:.0f} hours</span>"))
...
fig.update_layout(
    ...
    xaxis=dict(title="hours", range=[0, 104]),
    yaxis=dict(autorange="reversed",
    ...
    updatemenus=[menu_style(buttons=[
        ...
        restyle("developer-days", 13, "developer-days")])])
show(fig)
```

[**The estimate** comes from the spec, written before any code and never revised. **The measurement** counts only hands-on time — any gap longer than 45 minutes is excluded — and is rounded down.]{.caption}
...
So. The grey bar is what the spec said. Between sixty-eight and ninety-two hours of work. Written down before any code existed, and never revised afterwards.

The gold bar is what it actually took.

<span style="color:#c1121f;font-weight:700">[PAUSE — LET THEM READ THE GAP BEFORE YOU SAY THE NUMBER]</span>

Nine hours.

<span style="color:#c1121f;font-weight:700">[POINT AT THE BOX IN THE BOTTOM RIGHT. EMPHASIZE "NINE TIMES FASTER".]</span>

That's roughly nine times faster. Getting on for ninety percent less time.

Now, I want to be careful here, because this is exactly the kind of number people inflate.

<span style="color:#c1121f;font-weight:700">[POINT AT THE ERROR BAR ON THE GREY BAR]</span>

The estimate is a range, so I've drawn it. Depending on which end you take, it's somewhere between eight and ten times. I'd rather give you a range I can defend than one number I can't.

And that nine hours is hands-on time only. If I stepped away for more than about three quarters of an hour, I stopped counting. Then I rounded it down. So if anything that bar is generous to the traditional side.

<span style="color:#c1121f;font-weight:700">[CLICK THE "DEVELOPER-DAYS" BUTTON]</span>

Here it is the other way round, if days are easier to hold. Ten developer-days against a bit over one.

<span style="color:#c1121f;font-weight:700">[CLICK BACK TO "HOURS"]</span>

<span style="color:#c1121f;font-weight:700">[SLOW DOWN]</span>

And remember what I told you at the start. I'm not a software developer. So this isn't an experienced engineer going nine times faster than themselves. It's someone who could specify the system but couldn't have written most of it, getting to a finished, deployed, tested application anyway.
...
::: {.notes}
<span style="color:#c1121f;font-weight:700">[TARGET TIME: 0:30 — TOTAL 14:45]</span>
...
::: {style="font-family:'Bricolage Grotesque',sans-serif;font-weight:700;font-size:3.6em;line-height:1;letter-spacing:-0.04em;margin:0.1em 0 0 0"}
Thank you
:::

::: {style="margin-top:0.9em;font-size:0.52em;line-height:1.6;color:#8b929c;max-width:22em"}
Nine hours, three dollars a month, and a thing that actually runs.
:::

::: {.cols .cols-3 style="margin-top:1.5em"}
::: {}
[The app]{.kicker}
[ticker-dash.fly.dev](https://ticker-dash.fly.dev/){target="_blank" rel="noopener" style="font-family:'IBM Plex Mono',monospace;font-size:0.4em"}
:::
::: {}
[The code]{.kicker}
[github.com/IHoKo/<br>Live-Trading-Dashboard](https://github.com/IHoKo/Live-Trading-Dashboard){target="_blank" rel="noopener" style="font-family:'IBM Plex Mono',monospace;font-size:0.4em"}
:::
::: {}
[These slides]{.kicker}
[ihoko.github.io/<br>Quarto_Presentation](https://ihoko.github.io/Quarto_Presentation/){target="_blank" rel="noopener" style="font-family:'IBM Plex Mono',monospace;font-size:0.4em"}
:::
:::

::: {style="margin-top:1.4em;font-family:'IBM Plex Mono',monospace;font-size:0.36em;letter-spacing:0.14em;text-transform:uppercase;color:#8b929c"}
AAPL [319.97]{data-tick="319.97" style="color:#2f7d6e"} &nbsp;·&nbsp;
MSFT [499.70]{data-tick="499.70" style="color:#2f7d6e"} &nbsp;·&nbsp;
NVDA [230.36]{data-tick="230.36" style="color:#a8442f"}
:::
...
Everything's up there if you want to look at it yourself. The app is live, the code is public, and these slides are on that last link — including all the working, so you can check any number I've given you.

I'm very happy to take questions. And the app is still open if anyone wants to poke at it.

<span style="color:#c1121f;font-weight:700">[LEAVE THIS SLIDE UP FOR Q&A. IF YOU NEED THE APP AGAIN, PRESS LEFT ARROW BACK TO THE DEMO SLIDE.]</span>

<span style="color:#c1121f;font-weight:700">[PREPARED ANSWERS — DO NOT READ ALOUD:]</span>

<span style="color:#c1121f;font-weight:700">[IF ASKED "COULD I DO THIS?" → THE SPEC WAS THE HARD PART. WRITING A CLEAR SPEC IS A SKILL PLENTY OF PEOPLE HERE ALREADY HAVE.]</span>

<span style="color:#c1121f;font-weight:700">[IF ASKED ABOUT CODE QUALITY → 291 TESTS, A LINTER, A TYPE CHECKER, A REVIEW GATE ON EVERY PHASE. THEN POINT AT THE CACHE BUG THAT REVIEW CAUGHT AND THE TESTS DID NOT.]</span>

<span style="color:#c1121f;font-weight:700">[IF ASKED WHAT IT COST → ONE MONTH OF A CLAUDE SUBSCRIPTION, WHICH I PAY ANYWAY, PLUS ABOUT THREE DOLLARS A MONTH TO HOST.]</span>

<span style="color:#c1121f;font-weight:700">[IF PUSHED ON THE 9x → THE MEASURED SIDE IS THE CONSERVATIVE END AND ROUNDED DOWN; THE ESTIMATE IS MY OWN, WRITTEN BEFORE THE BUILD AND NEVER REVISED.]</span>

<span style="color:#c1121f;font-weight:700">[IF ASKED WHAT YOU WOULD DO DIFFERENTLY → DEPLOY EVEN EARLIER. THREE OF THE FOUR DEFECTS ONLY APPEARED IN PRODUCTION.]</span>
:::
